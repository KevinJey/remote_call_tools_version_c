...
void run_command(char *message){
    ...
    strcpy(deal_buffer,message);
    while(deal_buffer[k] && deal_buffer[k] != '>') k++;
    while(deal_buffer[k] && !isalpha(deal_buffer[k])) k++;
    while(deal_buffer[k] && deal_buffer[k] != '@'){
        // putchar(deal_buffer[k]);
        // putchar(';');
        shell_cmd[shell_size++] = deal_buffer[k];
        if(deal_buffer[k] == '@'){
            at_appear_mark = 1;
        }
        k++;
    ...
    system(shell_cmd);
    strcat(shell_cmd,"> output_buffer");
    // printf("shellcmd is %s\n",shell_cmd);
    system(shell_cmd);
}